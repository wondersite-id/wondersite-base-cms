    ...
    <form method="POST" action="{{ route('cms.'.$routePrefix . '.update', $model->id) }}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <textarea id="short_description" class="form-control @error('short_description') is-invalid @enderror" name="short_description">{{ old('short_description', $model->short_description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea id="content" class="wysiwyg form-control @error('content') is-invalid @enderror" name="content">{{ old('content', $model->content) }}</textarea>
                @error('content')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            <div class="form-group">
                ...
                <select class="form-control @error('article_type_id') is-invalid @enderror" id="article_type_id" name="article_type_id">
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type->id }}" {{ old('article_type_id') == $type->id || $model->article_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
